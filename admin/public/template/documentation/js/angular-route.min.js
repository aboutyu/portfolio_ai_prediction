  ...
  var x,
    ...
    s = d.module('ngRoute', ['ng']).provider('$route', function () {
      ...
      this.when = function (b, e) {
        ...
        d.isUndefined(a.caseInsensitiveMatch) &&
          (a.caseInsensitiveMatch = this.caseInsensitiveMatch);
        g[b] = d.extend(a, b && l(b, a));
        b &&
          ((c = '/' == b[b.length - 1] ? b.substr(0, b.length - 1) : b + '/'),
          (g[c] = d.extend({ redirectTo: b }, l(c, a))));
        return this;
      };
        ...
        function (b, e, a, c, k, p, l) {
          function h(a) {
            ...
            (x =
              ...
              f &&
              r.$$route === f.$$route &&
              d.equals(r.pathParams, f.pathParams) &&
              !r.reloadOnSearch &&
              !z) ||
              (!f && !r) ||
              (b.$broadcast('$routeChangeStart', r, f).defaultPrevented &&
                a &&
                a.preventDefault());
          }
          function n() {
            ...
            if (x)
              (u.params = f.params),
                d.copy(u.params, a),
                b.$broadcast('$routeUpdate', u);
            else if (f || u)
              (z = !1),
                (v.current = f) &&
                  f.redirectTo &&
                  (d.isString(f.redirectTo)
                    ? e
                        .path(w(f.redirectTo, f.params))
                        .search(f.params)
                        .replace()
                    : e
                        .url(f.redirectTo(f.pathParams, e.path(), e.search()))
                        .replace()),
                c
                  .when(f)
                  .then(m)
                  .then(
                    function (c) {
                      f == v.current &&
                        (f && ((f.locals = c), d.copy(f.params, a)),
                        b.$broadcast('$routeChangeSuccess', f, u));
                    },
                    function (a) {
                      f == v.current &&
          ...
          function s(a) {
            ...
            d.isDefined((b = a.template))
              ...
              : d.isDefined((c = a.templateUrl)) &&
                ...
                d.isDefined(c) &&
                  ((a.loadedTemplateUrl = l.valueOf(c)), (b = p(c))));
            return b;
          }
          function y() {
            var a, b;
              ...
              updateParams: function (a) {
                if (this.current && this.current.$$route)
                  (a = d.extend({}, this.current.params, a)),
                    e.path(w(this.current.$$route.originalPath, a)),
                    e.search(a);
                else throw D('norout');
              },
            };
